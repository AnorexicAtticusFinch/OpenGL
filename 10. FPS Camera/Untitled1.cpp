...
float camSpd = 0.05, delTime = 0, lastTime = 0;
...
{
    float xDel = xPos - xLast;
    float yDel = yLast - yPos;
    xLast = xPos;
    yLast = yPos;
    ...
    camDir.x = cos(glm::radians(pitch)) * sin(glm::radians(yaw));
    camDir.y = sin(glm::radians(pitch));
    camDir.z = cos(glm::radians(pitch)) * -1 * cos(glm::radians(yaw));
...
void processInputs(GLFWwindow* window)
{
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        camPos += camSpd * camDir;
    
    else if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        camPos -= camSpd * camDir;
    
    else if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
        camPos += camSpd * glm::normalize(glm::cross(camDir, camUp));
...
int main()
{
    ...
    float time, xVal, zVal, radius;
    radius = 10.0f;

    while (! glfwWindowShouldClose(window))
    {
        //cout << "Pitch and Yaw: " << pitch << "  " << yaw << endl;
